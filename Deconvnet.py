...
def DeconvNet(path, model_name, layer_name_DN, class_of_interest, input):
    '''
    :param path: the path where stored the model weights
    :param model_name: the model to be used
    :param layer_name_DN: the last convolutional layer
    ...
    dn_model = tf.keras.models.Model(
        [model.inputs],
        [model.get_layer(layer_name_DN).output, model.output])

    # 3.apply the new rule of ReLU to each ReLU layer
    layer_dict = [layer for layer in dn_model.layers[1:] if hasattr(layer, 'activation')]
    ...
    with tf.GradientTape() as tape:
        ...
        tape.watch(inputs)
        feature_map, logits = dn_model(inputs, training=False)
